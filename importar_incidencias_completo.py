"""
Script para importar incidencias desde Incidencias_Completo.xlsm a la base de datos
Preserva todos los datos reales exactamente como están clasificados
"""
import os
import django
import openpyxl
...
def parsear_fecha_hora(fecha_val, hora_val):
    """Combina fecha y hora del Excel en un datetime"""
    try:
        ...
        # Si falla, usar solo la fecha con hora 00:00
        if isinstance(fecha_val, datetime):
            return fecha_val
        return datetime.strptime(str(fecha_val).split()[0], '%Y-%m-%d')

def normalizar_centro(nombre_centro):
    """Normaliza nombres de centros del Excel al formato de la BD"""
    mapeo = {
        'TRAFUN': 'trafun',
        'TRAFÚN': 'trafun',
        'LIQUIÑE': 'liquine',
...
def buscar_operario(nombre_persona, cargo, centro_id):
    """Busca el operario en la BD por nombre o cargo"""
    ...
    try:
        ...
        # Buscar por cargo si no se encuentra por nombre
        if cargo:
            operario = Operario.objects.filter(
                centro=centro,
        ...
        # Si no se encuentra, usar el primer operario del centro
        return Operario.objects.filter(centro=centro).first()
    except:
        return None

def importar_hoja(ws, nombre_hoja):
    """Importa datos de una hoja del Excel"""
    print(f"\n{'='*80}")
    ...
    print(f"Columnas encontradas: {list(headers.values())}")
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        try:
            ...
            try:
                ...
                print(f"  [!] Centro no encontrado: {centro_nombre} (ID: {centro_id})")
                errores += 1
                continue
            ...
            # Convertir tiempo de resolución
            try:
            ...
            incidencia = Incidencia.objects.create(
                ...
                centro=centro,
                tipo_incidencia='modulos',  # Todas son de módulos según el Excel
                modulo=modulo,
                estanque=estanque,
                parametros_afectados=sensor.lower() if sensor else '',
                
                # Datos de oxígeno (la mayoría son oxígeno alto/bajo)
                oxigeno_nivel='alta' if 'alto' in tipo_incidencia.lower() else 'baja' if 'bajo' in tipo_incidencia.lower() else '',
                oxigeno_valor=valor if sensor and 'oxigeno' in sensor.lower() else '',
                
                # Datos de temperatura
                temperatura_nivel='alta' if 'temperatura' in tipo_incidencia.lower() and 'alta' in tipo_incidencia.lower() else 'baja' if 'temperatura' in tipo_incidencia.lower() and 'baja' in tipo_incidencia.lower() else '',
                temperatura_valor=valor if sensor and 'temperatura' in sensor.lower() else '',
...
def main():
    print("="*80)
    print("IMPORTANDO INCIDENCIAS DESDE Incidencias_Completo.xlsm")
    print("="*80)
    
    archivo = 'Incidencias_Completo.xlsm'
    
    # Limpiar incidencias existentes (opcional - comentar si quieres mantener las existentes)
    # Incidencia.objects.all().delete()
    # print("\n[!] Incidencias existentes eliminadas")
